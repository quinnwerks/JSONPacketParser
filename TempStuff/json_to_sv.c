...
// Pass in array
void svArrayPasser(const char * jsonFilePath, svOpenArrayHandle svData, svOpenArrayHandle svKeep, svLogic svLast, svOpenArrayHandle svNumFlit) {
    /*
    * STEPS
    * 1. create a packet list
    * 2. call parse JSON to build the packet list
    * 3. iterate through for loop to set equivalent openArrayHandles, and logics 
    * 4. call it a day and crack open a cold one
    * 5. add stuff to compilation header...
    */
   packet ext_packet_list[NUM_MAX_PACKETS];
   int zero = 0;
   parseJSON(jsonFilePath, ext_packet_list, &zero, &zero);
   for(unsigned i = 0; i < NUM_MAX_PACKETS; i++){
        for(unsigned j =0; j< ext_packet_list[i].num_flits; j++){
           ...
           svPutLogicArrElem2VecVal(svLast, ext_packet_list[i].flit_list[j].last, i,j);
        }
